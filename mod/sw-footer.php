...
presence.presence_date LIKE '$year%' AND presence.employees_id = '$idPegawaiXX'";
    ...
    $tglzlibur = $tglylibur[2] . "-" . $tglylibur[1] . "-" . $tglylibur[0];
     ...
     if(event.icon){          
        "; echo 'element.find(".fc-title").prepend("<i class=';echo "'fa fa-";echo '"+event.icon+"';echo "'></i>";echo'"';echo");
     }
  }
                });
...
<script>
  ...
  waitingDialog.show('Initializing Data Face Recognition....', {dialogSize: 'sm', progressType: 'success'})
  $("#parent1").hide();
  $("#parent2").hide();
  Promise.all([
  ...
  async function start() {
    $.ajax({
        datatype: 'json',
        url: "https://absensi.leuwimunding.my.id/fetch",
        data: "",
        headers : { 
  ...
  async function onPlay() {
      ...
      if(videoEl.paused || videoEl.ended )
        ...
        //   if(faceMatcher != undefined){
		    ...
		    const displaySize = { width: 590, height: 460 }
        ...
        if(counter <= 5 && counter >= 0 ){
          ...
          if( detections != null){
            ...
            if(counter == 0){
              ...
              $.ajax({
                  type: "POST",
                  url: "https://absensi.leuwimunding.my.id/ajax",
