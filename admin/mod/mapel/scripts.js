...
$('.add-mapel').submit(function (e) {
    if($('input[type=text]').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/mapel/proses.php?action=add",
            type: "POST",
            ...
            success: function (data) {
                if (data == 'success') {
                    swal({title: 'Berhasil!', text: 'Data Jadwal  berhasil disimpan.!', icon: 'success', timer: 1500,});
                   $('#modalAdd').modal('hide');
...
$('.update-lokasi').submit(function (e) {
    if($('#txtname').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/mapel/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"mod/mapel/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
