...
$('.add-lokasi').submit(function (e) {
    if($('input[type=text]').val()=='' && $('textarea.address').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/pengumuman/proses.php?action=add",
            type: "POST",
...
$('.update-lokasi').submit(function (e) {
    if($('#txttitle').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/pengumuman/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
            ...
            success: function (data) {
                if (data == 'success') {
                    swal({title: 'Berhasil!', text: 'Data Pengumuman berhasil disimpan.!', icon: 'success', timer: 1500,});
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"mod/pengumuman/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
