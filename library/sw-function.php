...
function countWork($year, $month, $ignore) {
    $count = 0;
...
function last_friday_of_month($year, $month) {
  ...
  $a_date = $year . "-" . $month . "-" . $day;
    $cobajah = date("w", strtotime($a_date));
    if ($cobajah <= 5){
        $wkd = $cobajah;
    }else
    {
...
function lastdatefromweekday($weekday, $year, $month, $day) {
		...
		if ($day <= $lastday) {
			$lastdate = date("Y-m-d", mktime(0, 0, 0, $month, $day + 7 * ($weekday - 1), $year));
		} else {
			$weekday = $weekday - 7;
			$lastdate = date("Y-m-d", mktime(0, 0, 0, $month + 1, $day + 7 * ($weekday - 1), $year));
		}
		return $lastdate;
	}
