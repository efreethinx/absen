...
if (empty($_GET['latitude'])) {
  ...
  if($result_u->num_rows > 0){
      ...
      if($result->num_rows > 0){
        ...
        $kordinatsekolah = '-6.828957028076009, 108.62091820819346';
        ...
        $row = $result->fetch_assoc();
          if($result->num_rows > 0){
            if($row['time_out']=='00:00:00'){
                ...
                if ($gettimer < 1530 ){
                    echo "Belum Waktunya Absen Pulang!, Absen Pulang Bisa Dilakukan Pukul 15:30 WIB";
                }else
                {
            ...
            echo'Sebelumnya "'.$row_user['employees_name'].'" sudah pernah Absen Pulang pada Tanggal '.tanggal_ind($date).' dan Jam '.$row['time_out'].'.!';
                    ...
                    }else
                    {
                        ...
                        {
                            echo 'Maaf Jarak Lebih Dari 100';
                        }
                        else
                        {
                            //$sx1 =  number_format((float)$jarak1, 14, '.', ''); 
        ...
        }else if ($meter >= 100)
        {
            ...
            // die;
             echo "Anda Tidak Berada Diwilayah Absen, Jarak Anda ".$meter." Meter";
        }
        else
        {
            $gettimes = explode(":" , $time);
